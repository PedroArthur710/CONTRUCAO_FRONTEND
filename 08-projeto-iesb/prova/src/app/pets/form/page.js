...
export default function PetFormPage(props) {

  const router = useRouter()

  // Recuperando lista de pets do localStorage
  const pets = JSON.parse(localStorage.getItem('pets')) || []

  // Recuperando id para edição, se existir
  ...
  function salvar(dados) {
    if (petEditado) {
      // Se o pet já existe, atualiza os dados no localStorage
      Object.assign(petEditado, dados)
      localStorage.setItem('pets', JSON.stringify(pets))
    } else {
      // Cria um novo pet e salva no localStorage
      dados.id = v4()
      pets.push(dados)
      localStorage.setItem('pets', JSON.stringify(pets))
    }
